...
const contactIcons = {
  email:
    '<svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right: 3px;"><path d="m22 7-8.991 5.727a2 2 0 0 1-2.009 0L2 7"/><rect x="2" y="4" width="20" height="16" rx="2"/></svg>',
  phone:
...
function buildAdvancedContactHtml(cv, t) {
  ...
  if (hasValue(cv.location))
    items.push(`<span>${contactIcons.location}${escapeHtml(cv.location)}</span>`);
  if (hasValue(cv.linkedin)) {
    const linkedinHref = cv.linkedin.startsWith("http")
      ? escapeHtml(cv.linkedin)
      : `https://${escapeHtml(cv.linkedin)}`;
    items.push(
      `<a href="${linkedinHref}" target="_blank" style="color:#555;text-decoration:none;">${contactIcons.linkedin}${escapeHtml(t.linkedinLabel || "LinkedIn")}</a>`
    );
  }
  if (hasValue(cv.website)) {
    const websiteHref = cv.website.startsWith("http")
      ? escapeHtml(cv.website)
      : `https://${escapeHtml(cv.website)}`;
    items.push(
      `<a href="${websiteHref}" target="_blank" style="color:#555;text-decoration:none;">${contactIcons.website}${escapeHtml(t.portfolioLabel || "Portfolio")}</a>`
    );
  }
  return items.join("&nbsp;&nbsp;&nbsp;");
}
  ...
  return `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .cv-name {
      ...
      ${isProfessional ? "text-transform: uppercase; letter-spacing: 1px;" : ""}
    }
...
function buildExperienceHtml(experiences, keepTogether, templateId, t = {}) {
  const filtered = experiences.filter((exp) => hasValue(exp.company) || hasValue(exp.position));
  ...
  const items = filtered
    .map((exp) => {
      ...
      if (isProfessional) {
        ...
        const rightParts = [formatDateRange(exp.startDate, exp.endDate), exp.location]
          .filter(hasValue)
          .map(escapeHtml);
        headerHtml = `
      ...
      if (keepTogether || bulletItems.length <= 1) {
        ...
        const remainingBullets = bulletItems
          .slice(1)
          .map((b, j) => {
...
function buildEducationHtml(education, t = {}) {
  const filtered = education.filter((edu) => hasValue(edu.school) || hasValue(edu.degree));

  if (filtered.length === 0) return "";

  const items = filtered
    .map((edu) => {
      const schoolLine = [edu.school, edu.location].filter(hasValue).map(escapeHtml).join(" | ");
...
function buildSkillsHtml(skills, templateId, t = {}) {
  const filtered = skills.filter((skill) => hasValue(skill.category) || hasValue(skill.items));
...
function buildVolunteeringHtml(volunteering, keepTogether, t = {}) {
  ...
  const items = filtered
    .map((item) => {
      const subtitle = [item.role, item.location].filter(hasValue).map(escapeHtml).join(" | ");
...
function buildReferencesHtml(visibleReferences, t = {}) {
  ...
  const items = visibleReferences
    .map((ref) => {
      const contact = [ref.phone, ref.email].filter(hasValue).map(escapeHtml).join(" | ");
